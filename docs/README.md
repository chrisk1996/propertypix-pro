# Zestio Pro Documentation

This folder contains technical and functional documentation for all modules of Zestio Pro.

## Documentation Structure

| Document | Description |
|----------|-------------|
| [architecture.md](./architecture.md) | System architecture, tech stack, and data flow |
| [modules.md](./modules.md) | Detailed module documentation (Enhance, Staging, Floor Planner, Video) |
| [api.md](./api.md) | API endpoints and specifications |
| [components.md](./components.md) | Reusable UI components |
| [database.md](./database.md) | Database schema and models |

## Quick Links

- [Project README](../README.md)
- [Main Application](../src/app/)
- [Components](../src/components/)
- [API Routes](../src/app/api/)

## Contributing

When adding new features, please update the relevant documentation files.

---
*Last updated: April 2026*
