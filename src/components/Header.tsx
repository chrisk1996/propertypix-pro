  ...
  return (
    <header className="fixed top-0 w-full z-50 bg-[#f7f9ff]/80 backdrop-blur-md border-b border-[#c4c6cd]/10">
      <div className="max-w-[1920px] mx-auto flex justify-between items-center px-12 py-5">
        {/* Logo */}
        <Link href="/" className="text-2xl font-serif italic text-[#1d2832]">
          PropertyPix
        </Link>
        ...
        <div className="hidden md:flex items-center gap-6">
          {user ? (
            ...
            <>
              ...
              <Link 
                href="/auth" 
                className="bg-[#1d2832] text-white px-8 py-2.5 text-xs font-manrope uppercase tracking-widest rounded hover:bg-[#333e48] transition-all"
              >
                Get Started
              </Link>
            </>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-[#f7f9ff] border-b border-[#c4c6cd]/10">
          <nav className="px-4 py-4 space-y-2">
            ...
            <span className="block px-4 py-2 text-[#1d2832] opacity-40 cursor-not-allowed font-manrope text-xs uppercase tracking-widest">
              Solutions
            </span>
            <Link 
              href="/pricing" 
